...
(function () {
  ...
  var cur = { url: null, to: null, ctrl: null, fulltext: null };
  ...
  function build() {
    ...
    el.innerHTML =
      ...
      '<div class="trans__panel" role="dialog" aria-modal="true" aria-labelledby="transTitleEl">' +
        ...
        '<div class="trans__tabs" role="tablist">' +
          '<button type="button" data-to="raw" id="transTabRaw"></button>' +
    ...
    document.getElementById("transTabRaw").addEventListener("click", function () { if (cur.url && cur.to !== "raw") load(cur.url, "raw"); });
  ...
  function close() {
    ...
    seq++;                                               // 使在飞回调全部失效
    cur = { url: null, to: null, ctrl: null, fulltext: null };
  }
  function setTabs(to) {
    document.getElementById("transTabRaw").className = to === "raw" ? "is-active" : "";
    document.getElementById("transTabZh").className = to === "zh" ? "is-active" : "";
    document.getElementById("transTabEn").className = to === "en" ? "is-active" : "";
    // 免责注随 tab:原文=存档声明;中/EN=机翻声明
    var note = document.getElementById("transNote");
    if (note) note.textContent = AP.t(to === "raw" ? "transNoteRaw" : "transNote");
  }
  function renderText(body, title, text, truncated) {
    var html = "";
    if (title) html += '<h3 class="trans__pagetitle">' + esc(title) + '</h3>';
    var parts = String(text || "").split(/\n+/);
    for (var i = 0; i < parts.length; i++) if (parts[i].trim()) html += "<p>" + esc(parts[i]) + "</p>";
    if (truncated) html += '<p class="trans__loading">' + esc(AP.t("transTruncated")) + '</p>';
  ...
  function load(url, to) {
    ...
    cur.ctrl = ctrl;
    // 原文模式:优先读入库时的存档(静态文件,秒开);没有存档(老数据)才实时抓原文
    if (to === "raw" && cur.fulltext) {
      fetch(cur.fulltext, ctrl ? { signal: ctrl.signal, cache: "force-cache" } : { cache: "force-cache" })
        .then(function (r) { if (!r.ok) throw new Error("nofile"); return r.text(); })
        .then(function (t) { if (myId === seq) renderText(body, "", t, false); })
        .catch(function () { if (myId === seq) apiLoad(); });   // 存档缺失 → 实时抓
      return;
    }
    apiLoad();
    function apiLoad() {
      ...
      fetch("/api/pagetrans?url=" + encodeURIComponent(url) + "&to=" + to, ctrl ? { signal: ctrl.signal } : {})
        ...
        .then(function (j) {
          ...
          renderText(body, j.title, j.text, j.truncated);
        })
        .catch(function () {
  ...
  AP.pageTrans = {
    open: function (url, fulltext) {
      if (!url) return;
      build();
      cur.fulltext = fulltext || null;
      document.getElementById("transTitleEl").textContent = AP.t("transTitle");
      document.getElementById("transTabRaw").textContent = AP.t("transTabRaw");
      ...
      load(url, "raw");                                  // 默认展示未精简的官网原文;中/EN 翻译按需点
    },
    close: close
  };
})();

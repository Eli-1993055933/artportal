...
// 统一入口。返回 { data, usage, raw }
export async function extract(sourceText, ctx) {
  const prompt = await getPrompt();
  const user = buildUserContent(sourceText.slice(0, MAX_INPUT_CHARS), ctx);
  if (DEEPSEEK_KEY) return extractDeepSeek(prompt, user);
  if (ANTHROPIC_KEY) return extractAnthropic(prompt, user);
...
async function extractDeepSeek(system, user) {
...
async function extractAnthropic(system, user) {
